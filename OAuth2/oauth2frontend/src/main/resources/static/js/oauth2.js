...
const CLIENT_ID = "oauth2-client"; // название должен совпадать c клиентом из KeyCloak
...
const GRANT_TYPE_REFRESH_TOKEN = "refresh_token"; // для обмена refresh token
...
const KEYCLOAK_URI = "http://localhost:8180/realms/oauth2-apprealm/protocol/openid-connect"; // общий URI KeyCloak
...
function initAccessToken() {
    ...
    // document.getElementById("originalState").innerText = state;
    // console.log("state = " + state)
    localStorage.setItem(STATE_KEY, state);


    var codeVerifier = generateCodeVerifier();
    // document.getElementById("codeVerifier").innerText = codeVerifier;
...
function requestAuthCode(state, codeChallenge) {
    ...
    window.open(authUrl, 'self'); // открываем в этом же окне (self) окно авторизации KeyCloak
}
...
function requestTokens(stateFromAuthServer, authCode) { // idea может показывать, что функция нидге не используется, но это не так, просто он не может определить вызов из другого window
    ...
    // var originalState = document.getElementById("originalState").innerText;
...
function accessTokenResponse(data, status, jqXHR) { // // эти параметры передаются автоматически, data будет в формате JSON
    ...
    document.getElementById("email").innerText = "Привет: " + payload["email"];
